...
ALboolean _alutInputStreamDestroy(InputStream *stream) {
  ALboolean status = (stream->isFileStream && fclose(stream->u.fileDescriptor))
                         ? AL_FALSE
                         : AL_TRUE;
  if (stream->fileName) {
...
static ALboolean streamRead(InputStream *stream, void *ptr,
...
ALboolean _alutInputStreamReadInt32BE(InputStream *stream,
  ...
  *value = ((Int32BigEndian)buf[0] << 24) | ((Int32BigEndian)buf[1] << 16) |
...
ALboolean _alutInputStreamReadUInt32LE(InputStream *stream,
  ...
  *value = ((UInt32LittleEndian)buf[3] << 24) |
