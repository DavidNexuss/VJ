...
#define IM_ASSERT(_EXPR) ENGINE_ASSERT(_EXPR)
...
// window: ShowMetricsWindow() will be empty.
...
//---- Use stb_printf's faster implementation of vsnprintf instead of the one
// from libc (unless IMGUI_DISABLE_DEFAULT_FORMAT_FUNCTIONS is defined)
// Requires 'stb_sprintf.h' to be available in the include path. Compatibility
// checks of arguments and formats done by clang and GCC will be disabled in
// order to support the extra formats provided by STB sprintf. #define
