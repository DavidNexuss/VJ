...
lm_context *lmCreate(
    int hemisphereSize,      // hemisphereSize: resolution of the hemisphere
                             // renderings. must be a power of two! typical: 64.
    float zNear, float zFar, // zNear/zFar: hemisphere min/max draw distances.
    float clearR, float clearG,
    float clearB, // clear color / background color / sky color.
    int interpolationPasses,
    float
        interpolationThreshold, // passes: hierarchical selective interpolation
                                // passes (0-8; initial step size = 2^passes).
                                // threshold: error value below which lightmap
                                // pixels are interpolated instead of rendered.
                                // use output image from LM_DEBUG_INTERPOLATION
                                // to determine a good value. values around and
                                // below 0.01 are probably ok. the lower the
                                // value, the more hemispheres are rendered ->
                                // slower, but possibly better quality.
    float cameraToSurfaceDistanceModifier LM_DEFAULT_VALUE(
        0.0f)); // modifier for the height of the rendered hemispheres above the
                // surface -1.0f => stick to surface, 0.0f => minimum height for
                // interpolated surface normals, > 0.0f => improves gradients on
                // surfaces with interpolated normals due to the flat surface
                // horizon, but may introduce other artifacts.

// optional: set material characteristics by specifying cos(theta)-dependent
// weights for incoming light.
typedef float (*lm_weight_func)(float cos_theta, void *userdata);
void lmSetHemisphereWeights(
    lm_context *ctx, lm_weight_func f,
    void *userdata); // precalculates weights for incoming light depending on
                     // its angle. (default: all weights are 1.0f)

// specify an output lightmap image buffer with w * h * c * sizeof(float) bytes
// of memory.
void lmSetTargetLightmap(
    lm_context *ctx, float *outLightmap, int w, int h,
    int c); // output HDR lightmap (linear 32bit float channels; c:
            // 1->Greyscale, 2->Greyscale+Alpha, 3->RGB, 4->RGBA).

// set the geometry to map to the currently set target lightmap (set the target
// lightmap before calling this!).
void lmSetGeometry(
    lm_context *ctx,
    const float *transformationMatrix, // 4x4 object-to-world transform for the
                                       // geometry or NULL (no transformation).
    lm_type positionsType, const void *positionsXYZ,
    int positionsStride, // triangle mesh in object space.
    lm_type normalsType, const void *normalsXYZ,
    int normalsStride, // optional normals for the mesh in object space (Use
                       // LM_NONE type in case you only need flat surfaces).
    lm_type lightmapCoordsType, const void *lightmapCoordsUV,
    int lightmapCoordsStride, // lightmap atlas texture coordinates for the mesh
                              // [0..1]x[0..1] (integer types are normalized to
                              // 0..1 range).
    int count, lm_type indicesType LM_DEFAULT_VALUE(LM_NONE),
    const void *indices
        LM_DEFAULT_VALUE(0)); // if mesh indices are used, count = number of
                              // indices else count = number of vertices.
...
lm_bool lmBegin(
    lm_context *ctx,
    int *outViewport4, // output of the current viewport: { x, y, w, h }. use
                       // these to call glViewport()!
...
float lmProgress(
    lm_context *ctx); // should only be called between lmBegin/lmEnd!
                      // provides the light mapping progress as a value
...
// image based post processing (c is the number of color channels in the image,
// m a channel mask for the operation)
#define LM_ALL_CHANNELS 0x0f
float lmImageMin(
    const float *image, int w, int h, int c,
    int m LM_DEFAULT_VALUE(LM_ALL_CHANNELS)); // find the minimum value (across
                                              // the specified channels)
float lmImageMax(
    const float *image, int w, int h, int c,
    int m LM_DEFAULT_VALUE(LM_ALL_CHANNELS)); // find the maximum value (across
                                              // the specified channels)
void lmImageAdd(float *image, int w, int h, int c, float value,
                int m LM_DEFAULT_VALUE(
                    LM_ALL_CHANNELS)); // in-place add to the specified channels
void lmImageScale(
    float *image, int w, int h, int c, float factor,
    int m LM_DEFAULT_VALUE(
        LM_ALL_CHANNELS)); // in-place scaling of the specified channels
void lmImagePower(float *image, int w, int h, int c, float exponent,
                  int m LM_DEFAULT_VALUE(
                      LM_ALL_CHANNELS)); // in-place powf(v, exponent) of the
                                         // specified channels (for gamma)
void lmImageDilate(const float *image, float *outImage, int w, int h,
                   int c); // widen the populated non-zero areas by 1 pixel.
void lmImageSmooth(const float *image, float *outImage, int w, int h,
                   int c); // simple box filter on only the non-zero values.
void lmImageDownsample(const float *image, float *outImage, int w, int h,
                       int c); // downsamples [0..w]x[0..h] to [0..w/2]x[0..h/2]
                               // by avereging only the non-zero values
void lmImageFtoUB(
    const float *image, unsigned char *outImage, int w, int h, int c,
    float max LM_DEFAULT_VALUE(
        0.0f)); // casts a floating point image to an 8bit/channel image

// TGA file output helpers
lm_bool lmImageSaveTGAub(const char *filename, const unsigned char *image,
                         int w, int h, int c);
lm_bool lmImageSaveTGAf(const char *filename, const float *image, int w, int h,
                        int c, float max LM_DEFAULT_VALUE(0.0f));
...
static inline float lm_pmodf(float a, float b) {
  return (a < 0.0f ? 1.0f : 0.0f) + (float)fmod(a, b);
} // positive mod

typedef struct lm_ivec2 {
  int x, y;
} lm_ivec2;
static inline lm_ivec2 lm_i2(int x, int y) {
  lm_ivec2 v = {x, y};
  return v;
}
...
static inline lm_vec2 lm_v2i(int x, int y) {
  lm_vec2 v = {(float)x, (float)y};
  return v;
}
...
static inline lm_vec2 lm_add2(lm_vec2 a, lm_vec2 b) {
  return lm_v2(a.x + b.x, a.y + b.y);
}
static inline lm_vec2 lm_sub2(lm_vec2 a, lm_vec2 b) {
  return lm_v2(a.x - b.x, a.y - b.y);
}
static inline lm_vec2 lm_mul2(lm_vec2 a, lm_vec2 b) {
  return lm_v2(a.x * b.x, a.y * b.y);
}
static inline lm_vec2 lm_scale2(lm_vec2 a, float b) {
  return lm_v2(a.x * b, a.y * b);
}
static inline lm_vec2 lm_div2(lm_vec2 a, float b) {
  return lm_scale2(a, 1.0f / b);
}
static inline lm_vec2 lm_pmod2(lm_vec2 a, float b) {
  return lm_v2(lm_pmodf(a.x, b), lm_pmodf(a.y, b));
}
static inline lm_vec2 lm_min2(lm_vec2 a, lm_vec2 b) {
  return lm_v2(lm_minf(a.x, b.x), lm_minf(a.y, b.y));
}
static inline lm_vec2 lm_max2(lm_vec2 a, lm_vec2 b) {
  return lm_v2(lm_maxf(a.x, b.x), lm_maxf(a.y, b.y));
}
static inline lm_vec2 lm_abs2(lm_vec2 a) {
  return lm_v2(lm_absf(a.x), lm_absf(a.y));
}
static inline lm_vec2 lm_floor2(lm_vec2 a) {
  return lm_v2(floorf(a.x), floorf(a.y));
}
static inline lm_vec2 lm_ceil2(lm_vec2 a) {
  return lm_v2(ceilf(a.x), ceilf(a.y));
}
static inline float lm_dot2(lm_vec2 a, lm_vec2 b) {
...
static inline lm_vec2 lm_normalize2(lm_vec2 a) {
  return lm_div2(a, lm_length2(a));
}
static inline lm_bool lm_finite2(lm_vec2 a) {
  return lm_finite(a.x) && lm_finite(a.y);
}

typedef struct lm_vec3 {
  float x, y, z;
} lm_vec3;
static inline lm_vec3 lm_v3(float x, float y, float z) {
  lm_vec3 v = {x, y, z};
  return v;
}
static inline lm_vec3 lm_negate3(lm_vec3 a) { return lm_v3(-a.x, -a.y, -a.z); }
static inline lm_vec3 lm_add3(lm_vec3 a, lm_vec3 b) {
  return lm_v3(a.x + b.x, a.y + b.y, a.z + b.z);
}
static inline lm_vec3 lm_sub3(lm_vec3 a, lm_vec3 b) {
  return lm_v3(a.x - b.x, a.y - b.y, a.z - b.z);
}
static inline lm_vec3 lm_mul3(lm_vec3 a, lm_vec3 b) {
  return lm_v3(a.x * b.x, a.y * b.y, a.z * b.z);
}
static inline lm_vec3 lm_scale3(lm_vec3 a, float b) {
  return lm_v3(a.x * b, a.y * b, a.z * b);
}
static inline lm_vec3 lm_div3(lm_vec3 a, float b) {
  return lm_scale3(a, 1.0f / b);
}
static inline lm_vec3 lm_pmod3(lm_vec3 a, float b) {
  return lm_v3(lm_pmodf(a.x, b), lm_pmodf(a.y, b), lm_pmodf(a.z, b));
}
static inline lm_vec3 lm_min3(lm_vec3 a, lm_vec3 b) {
  return lm_v3(lm_minf(a.x, b.x), lm_minf(a.y, b.y), lm_minf(a.z, b.z));
}
static inline lm_vec3 lm_max3(lm_vec3 a, lm_vec3 b) {
  return lm_v3(lm_maxf(a.x, b.x), lm_maxf(a.y, b.y), lm_maxf(a.z, b.z));
}
static inline lm_vec3 lm_abs3(lm_vec3 a) {
  return lm_v3(lm_absf(a.x), lm_absf(a.y), lm_absf(a.z));
}
static inline lm_vec3 lm_floor3(lm_vec3 a) {
  return lm_v3(floorf(a.x), floorf(a.y), floorf(a.z));
}
static inline lm_vec3 lm_ceil3(lm_vec3 a) {
  return lm_v3(ceilf(a.x), ceilf(a.y), ceilf(a.z));
}
static inline float lm_dot3(lm_vec3 a, lm_vec3 b) {
  return a.x * b.x + a.y * b.y + a.z * b.z;
}
static inline lm_vec3 lm_cross3(lm_vec3 a, lm_vec3 b) {
  return lm_v3(a.y * b.z - b.y * a.z, a.z * b.x - b.z * a.x,
               a.x * b.y - b.x * a.y);
}
static inline float lm_length3sq(lm_vec3 a) {
  return a.x * a.x + a.y * a.y + a.z * a.z;
}
static inline float lm_length3(lm_vec3 a) { return sqrtf(lm_length3sq(a)); }
static inline lm_vec3 lm_normalize3(lm_vec3 a) {
  return lm_div3(a, lm_length3(a));
}
static inline lm_bool lm_finite3(lm_vec3 a) {
  return lm_finite(a.x) && lm_finite(a.y) && lm_finite(a.z);
}
...
// this modifies the poly array! the poly array must be big enough to hold the
// result! res must be big enough to hold the result!
static int lm_convexClip(lm_vec2 *poly, int nPoly, const lm_vec2 *clip,
...
// pass order of one 4x4 interpolation patch for two interpolation steps (and
// the next neighbors right of/below it) 0 4 1 4 0 5 6 5 6 5 2 4 3 4 2 5 6 5 6 5
// 0 4 1 4 0

static unsigned int lm_passStepSize(lm_context *ctx) {
  unsigned int shift =
      ctx->meshPosition.passCount / 3 - (ctx->meshPosition.pass - 1) / 3;
...
static void
lm_moveToNextPotentialConservativeTriangleRasterizerPosition(lm_context *ctx) {
  ...
  while (ctx->meshPosition.rasterizer.x >= ctx->meshPosition.rasterizer.maxx) {
    ctx->meshPosition.rasterizer.x =
        ctx->meshPosition.rasterizer.minx + lm_passOffsetX(ctx);
...
static float *lm_getLightmapPixel(lm_context *ctx, int x, int y) {
  assert(x >= 0 && x < ctx->lightmap.width && y >= 0 &&
         y < ctx->lightmap.height);
  return ctx->lightmap.data +
         (y * ctx->lightmap.width + x) * ctx->lightmap.channels;
}

static void lm_setLightmapPixel(lm_context *ctx, int x, int y, float *in) {
  assert(x >= 0 && x < ctx->lightmap.width && y >= 0 &&
         y < ctx->lightmap.height);
  float *p = ctx->lightmap.data +
             (y * ctx->lightmap.width + x) * ctx->lightmap.channels;
...
static lm_bool
lm_trySamplingConservativeTriangleRasterizerPosition(lm_context *ctx) {
  if (lm_hasConservativeTriangleRasterizerFinished(ctx))
    return LM_FALSE;

  // check if lightmap pixel was already set
  float *pixelValue = lm_getLightmapPixel(ctx, ctx->meshPosition.rasterizer.x,
  ...
  lm_vec2 pixel[16];
  pixel[0] =
      lm_v2i(ctx->meshPosition.rasterizer.x, ctx->meshPosition.rasterizer.y);
  pixel[1] = lm_v2i(ctx->meshPosition.rasterizer.x + 1,
                    ctx->meshPosition.rasterizer.y);
  pixel[2] = lm_v2i(ctx->meshPosition.rasterizer.x + 1,
                    ctx->meshPosition.rasterizer.y + 1);
  pixel[3] = lm_v2i(ctx->meshPosition.rasterizer.x,
                    ctx->meshPosition.rasterizer.y + 1);
  ...
  lm_vec2 uv = lm_toBarycentric(ctx->meshPosition.triangle.uv[0],
                                ctx->meshPosition.triangle.uv[1],
                                ctx->meshPosition.triangle.uv[2], centroid);
  ...
  if (ctx->meshPosition.pass > 0) {
    ...
    {
      neighborsExpected += 2;
      if (ctx->meshPosition.rasterizer.x - d >=
              ctx->meshPosition.rasterizer.minx &&
          ctx->meshPosition.rasterizer.x + d <=
              ctx->meshPosition.rasterizer.maxx) {
        neighbors[neighborCount++] =
            lm_getLightmapPixel(ctx, ctx->meshPosition.rasterizer.x - d,
                                ctx->meshPosition.rasterizer.y);
        neighbors[neighborCount++] =
            lm_getLightmapPixel(ctx, ctx->meshPosition.rasterizer.x + d,
                                ctx->meshPosition.rasterizer.y);
    ...
    {
      neighborsExpected += 2;
      if (ctx->meshPosition.rasterizer.y - d >=
              ctx->meshPosition.rasterizer.miny &&
          ctx->meshPosition.rasterizer.y + d <=
              ctx->meshPosition.rasterizer.maxy) {
        neighbors[neighborCount++] =
            lm_getLightmapPixel(ctx, ctx->meshPosition.rasterizer.x,
                                ctx->meshPosition.rasterizer.y - d);
        neighbors[neighborCount++] =
            lm_getLightmapPixel(ctx, ctx->meshPosition.rasterizer.x,
                                ctx->meshPosition.rasterizer.y + d);
    ...
    if (neighborCount ==
        neighborsExpected) // are all interpolation neighbors available?
    {
      ...
      // check if error from average pixel to neighbors is above the
      ...
      if (interpolate) {
        lm_setLightmapPixel(ctx, ctx->meshPosition.rasterizer.x,
                            ctx->meshPosition.rasterizer.y, avg);
#ifdef LM_DEBUG_INTERPOLATION
        // set interpolated pixel to green in debug output
        ctx->lightmap
            .debug[(ctx->meshPosition.rasterizer.y * ctx->lightmap.width +
                    ctx->meshPosition.rasterizer.x) *
                       3 +
                   1] = 255;
#endif
  ...
  ctx->meshPosition.sample.position =
      lm_add3(p0, lm_add3(lm_scale3(v2, uv.x), lm_scale3(v1, uv.y)));
  ...
  ctx->meshPosition.sample.direction = lm_normalize3(
      lm_add3(n0, lm_add3(lm_scale3(nv2, uv.x), lm_scale3(nv1, uv.y))));
  ctx->meshPosition.sample.direction =
      lm_normalize3(ctx->meshPosition.sample.direction);
  float cameraToSurfaceDistance =
      (1.0f + ctx->hemisphere.cameraToSurfaceDistanceModifier) *
      ctx->hemisphere.zNear * sqrtf(2.0f);
  ctx->meshPosition.sample.position = lm_add3(
      ctx->meshPosition.sample.position,
      lm_scale3(ctx->meshPosition.sample.direction, cameraToSurfaceDistance));

  if (!lm_finite3(ctx->meshPosition.sample.position) ||
      !lm_finite3(ctx->meshPosition.sample.direction) ||
      lm_length3sq(ctx->meshPosition.sample.direction) <
          0.5f) // don't allow 0.0f. should always be ~1.0f
    return LM_FALSE;
  ...
  lm_vec3 side =
      lm_normalize3(lm_cross3(up, ctx->meshPosition.sample.direction));
  ...
  float phi = 2.0f * lm_pi * lm_baseAngles[ry][rx] +
              0.1f * ((float)rand() / (float)RAND_MAX);
  ctx->meshPosition.sample.up = lm_normalize3(
      lm_add3(lm_scale3(side, cosf(phi)), lm_scale3(up, sinf(phi))));
  return LM_TRUE;
#endif
}
...
static lm_bool
lm_findFirstConservativeTriangleRasterizerPosition(lm_context *ctx) {
...
static lm_bool
lm_findNextConservativeTriangleRasterizerPosition(lm_context *ctx) {
  lm_moveToNextPotentialConservativeTriangleRasterizerPosition(ctx);
...
static void lm_integrateHemisphereBatch(lm_context *ctx) {
  ...
  glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                         ctx->hemisphere.fbTexture[fbWrite], 0);
  glViewport(0, 0, outHemiSize * ctx->hemisphere.fbHemiCountX,
             outHemiSize * ctx->hemisphere.fbHemiCountY);
  ...
  glCopyTexSubImage2D(GL_TEXTURE_2D, 0, ctx->hemisphere.storage.writePosition.x,
                      ctx->hemisphere.storage.writePosition.y, 0, 0,
                      ctx->hemisphere.fbHemiCountX,
                      ctx->hemisphere.fbHemiCountY);
  glBindTexture(GL_TEXTURE_2D, 0);
  ...
  for (unsigned int y = 0; y < ctx->hemisphere.fbHemiCountY; y++) {
    ...
    for (unsigned int x = 0; x < ctx->hemisphere.fbHemiCountX; x++) {
      ...
      if (hemiIndex >= ctx->hemisphere.fbHemiIndex)
        ctx->hemisphere.storage
            .toLightmapLocation[sy * ctx->lightmap.width + sx] = lm_i2(-1, -1);
      else
        ctx->hemisphere.storage
            .toLightmapLocation[sy * ctx->lightmap.width + sx] =
  ...
  if (ctx->hemisphere.storage.writePosition.x +
          (int)ctx->hemisphere.fbHemiCountX >
      ctx->lightmap.width) {
    ctx->hemisphere.storage.writePosition.x = 0;
    ctx->hemisphere.storage.writePosition.y += ctx->hemisphere.fbHemiCountY;
    assert(ctx->hemisphere.storage.writePosition.y +
               (int)ctx->hemisphere.fbHemiCountY <
           ctx->lightmap.height);
  }
...
static void lm_writeResultsToLightmap(lm_context *ctx) {
  ...
  float *hemi = (float *)LM_CALLOC(ctx->lightmap.width * ctx->lightmap.height,
                                   4 * sizeof(float));
  glBindTexture(GL_TEXTURE_2D, ctx->hemisphere.storage.texture);
  glGetTexImage(GL_TEXTURE_2D, 0, GL_RGBA, GL_FLOAT, hemi);

  // write results to lightmap texture
  for (int y = 0; y < ctx->hemisphere.storage.writePosition.y +
                          (int)ctx->hemisphere.fbHemiCountY;
       y++) {
    for (int x = 0; x < ctx->lightmap.width; x++) {
      lm_ivec2 lmUV = ctx->hemisphere.storage
                          .toLightmapLocation[y * ctx->lightmap.width + x];
      if (lmUV.x >= 0) {
        float *c = hemi + (y * ctx->lightmap.width + x) * 4;
        float validity = c[3];
        float *lm =
            ctx->lightmap.data +
            (lmUV.y * ctx->lightmap.width + lmUV.x) * ctx->lightmap.channels;
        if (!lm[0] && validity > 0.9) {
          ...
          ctx->lightmap.debug[(lmUV.y * ctx->lightmap.width + lmUV.x) * 3 + 0] =
              255;
#endif
        }
      }
      ctx->hemisphere.storage.toLightmapLocation[y * ctx->lightmap.width + x]
          .x = -1; // reset
    }
  }
...
static void lm_setView(int *viewport, int x, int y, int w, int h, float *view,
                       lm_vec3 pos, lm_vec3 dir, lm_vec3 up, float *proj,
  ...
  view[10] = dir.z;
  view[11] = 0.0f;
  view[12] = lm_dot3(side, pos);
  view[13] = lm_dot3(up, pos);
  view[14] = lm_dot3(dir, pos);
  view[15] = 1.0f;

  // projection matrix: frustum(l, r, b, t, n, f)
  float ilr = 1.0f / (r - l), ibt = 1.0f / (t - b), ninf = -1.0f / (f - n),
        n2 = 2.0f * n;
  proj[0] = n2 * ilr;
  proj[1] = 0.0f;
  proj[2] = 0.0f;
...
static lm_bool lm_beginSampleHemisphere(lm_context *ctx, int *viewport,
  ...
  if (ctx->meshPosition.hemisphere.side == 0) {
    ...
    if (ctx->hemisphere.fbHemiIndex == 0) {
      ...
      glClearColor( // clear to valid background pixels!
          ctx->hemisphere.clearColor.r, ctx->hemisphere.clearColor.g,
  ...
  int x = (ctx->hemisphere.fbHemiIndex % ctx->hemisphere.fbHemiCountX) *
          ctx->hemisphere.size * 3;
  int y = (ctx->hemisphere.fbHemiIndex / ctx->hemisphere.fbHemiCountX) *
          ctx->hemisphere.size;
  ...
  case 0: // center
    lm_setView(viewport, x, y, size, size, view, pos, dir, up, proj, -zNear,
               zNear, -zNear, zNear, zNear, zFar);
    break;
  case 1: // right
    lm_setView(viewport, size + x, y, size / 2, size, view, pos, right, up,
               proj, -zNear, 0.0f, -zNear, zNear, zNear, zFar);
    break;
  case 2: // left
    lm_setView(viewport, size + x + size / 2, y, size / 2, size, view, pos,
               lm_negate3(right), up, proj, 0.0f, zNear, -zNear, zNear, zNear,
               zFar);
    break;
  case 3: // down
    lm_setView(viewport, 2 * size + x, y + size / 2, size, size / 2, view, pos,
               lm_negate3(up), dir, proj, -zNear, zNear, 0.0f, zNear, zNear,
               zFar);
    break;
  case 4: // up
    lm_setView(viewport, 2 * size + x, y, size, size / 2, view, pos, up,
               lm_negate3(dir), proj, -zNear, zNear, -zNear, 0.0f, zNear, zFar);
    break;
  default:
    assert(LM_FALSE);
    break;
...
static void lm_endSampleHemisphere(lm_context *ctx) {
  if (++ctx->meshPosition.hemisphere.side == 5) {
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
    if (++ctx->hemisphere.fbHemiIndex ==
        ctx->hemisphere.fbHemiCountX * ctx->hemisphere.fbHemiCountY) {
...
static lm_vec3 lm_transformPosition(const float *m, lm_vec3 v) {
  ...
  assert(lm_absf(d - 1.0f) < 0.00001f); // could divide by d, but this shouldn't
                                        // be a projection transform!
  return r;
}

static void lm_setMeshPosition(lm_context *ctx,
                               unsigned int indicesTriangleBaseIndex) {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    case LM_UNSIGNED_BYTE:
      vIndex = ((const unsigned char *)ctx->mesh.indices +
                ctx->meshPosition.triangle.baseIndex)[i];
      break;
    case LM_UNSIGNED_SHORT:
      vIndex = ((const unsigned short *)ctx->mesh.indices +
                ctx->meshPosition.triangle.baseIndex)[i];
      break;
    case LM_UNSIGNED_INT:
      vIndex = ((const unsigned int *)ctx->mesh.indices +
                ctx->meshPosition.triangle.baseIndex)[i];
      break;
    default:
      assert(LM_FALSE);
    ...
    ctx->meshPosition.triangle.p[i] =
        lm_transformPosition(ctx->mesh.modelMatrix, p);
    ...
    ctx->meshPosition.triangle.uv[i] = lm_mul2(
        lm_pmod2(uv, 1.0f), uvScale); // maybe clamp to 0.0-1.0 instead of pmod?
  ...
  lm_vec3 flatNormal = lm_cross3(
      ...
      lm_sub3(ctx->meshPosition.triangle.p[2],
              ctx->meshPosition.triangle.p[0]));

  for (int i = 0; i < 3; i++) {
    // decode and pre-transform vertex normal
    const void *nPtr =
        ctx->mesh.normals + vIndices[i] * ctx->mesh.normalsStride;
    lm_vec3 n;
    ...
    ctx->meshPosition.triangle.n[i] =
        lm_normalize3(lm_transformNormal(ctx->mesh.normalMatrix, n));
  }
  ...
  ctx->meshPosition.rasterizer.maxx =
      lm_mini((int)bbMax.x + 1, ctx->lightmap.width - 1);
  ctx->meshPosition.rasterizer.maxy =
      lm_mini((int)bbMax.y + 1, ctx->lightmap.height - 1);
  assert(
      ctx->meshPosition.rasterizer.minx <= ctx->meshPosition.rasterizer.maxx &&
      ctx->meshPosition.rasterizer.miny <= ctx->meshPosition.rasterizer.maxy);
  ctx->meshPosition.rasterizer.x =
      ctx->meshPosition.rasterizer.minx + lm_passOffsetX(ctx);
  ctx->meshPosition.rasterizer.y =
      ctx->meshPosition.rasterizer.miny + lm_passOffsetY(ctx);
  ...
  if (ctx->meshPosition.rasterizer.x <= ctx->meshPosition.rasterizer.maxx &&
    ...
    ctx->meshPosition.hemisphere.side =
        0; // we can start sampling the hemisphere
  else
    ctx->meshPosition.hemisphere.side =
        5; // no samples on this triangle! put hemisphere sampler into finished
           // state
}
...
static float lm_defaultWeights(float cos_theta, void *userdata) { return 1.0f; }

lm_context *lmCreate(int hemisphereSize, float zNear, float zFar, float clearR,
                     float clearG, float clearB, int interpolationPasses,
                     float interpolationThreshold,
                     float cameraToSurfaceDistanceModifier) {
  assert(hemisphereSize == 512 || hemisphereSize == 256 ||
         hemisphereSize == 128 || hemisphereSize == 64 ||
         hemisphereSize == 32 || hemisphereSize == 16);
  ...
  ctx->hemisphere.cameraToSurfaceDistanceModifier =
  ...
  unsigned int w[] = {ctx->hemisphere.fbHemiCountX * ctx->hemisphere.size * 3,
                      ctx->hemisphere.fbHemiCountX * ctx->hemisphere.size / 2};
  unsigned int h[] = {ctx->hemisphere.fbHemiCountY * ctx->hemisphere.size,
  ...
  glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
                            GL_RENDERBUFFER, ctx->hemisphere.fbDepth);
  for (int i = 0; i < 2; i++) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, w[i], h[i], 0, GL_RGBA, GL_FLOAT,
                 0);

    glBindFramebuffer(GL_FRAMEBUFFER, ctx->hemisphere.fb[i]);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                           ctx->hemisphere.fbTexture[i], 0);
  ...
  // hemisphere shader (weighted downsampling of the 3x1 hemisphere layout to a
  // 0.5x0.5 square)
  {
    const char *vs = "#version 150 core\n"
                     "const vec2 ps[4] = vec2[](vec2(1, -1), vec2(1, 1), "
                     "vec2(-1, -1), vec2(-1, 1));\n"
                     "void main()\n"
                     "{\n"
    ...
    const char *fs =
        ...
        "void main()\n"
        "{\n" // this is a weighted sum downsampling pass (alpha component
              // contains the weighted valid sample count)
    ...
    if (!ctx->hemisphere.firstPass.programID) {
      fprintf(stderr, "Error loading the hemisphere first pass shader "
                      "program... leaving!\n");
    ...
    ctx->hemisphere.firstPass.hemispheresTextureID = glGetUniformLocation(
        ctx->hemisphere.firstPass.programID, "hemispheres");
    ctx->hemisphere.firstPass.weightsTextureID =
        glGetUniformLocation(ctx->hemisphere.firstPass.programID, "weights");
  }
  ...
  // downsample shader
  {
    ...
    const char *fs =
        ...
        "void main()\n"
        "{\n" // this is a sum downsampling pass (alpha component contains the
              // weighted valid sample count)
    ...
    if (!ctx->hemisphere.downsamplePass.programID) {
      fprintf(stderr, "Error loading the hemisphere downsample pass shader "
                      "program... leaving!\n");
    ...
    ctx->hemisphere.downsamplePass.hemispheresTextureID = glGetUniformLocation(
        ctx->hemisphere.downsamplePass.programID, "hemispheres");
  }
  ...
  ctx->hemisphere.fbHemiToLightmapLocation = (lm_ivec2 *)LM_CALLOC(
      ctx->hemisphere.fbHemiCountX * ctx->hemisphere.fbHemiCountY,
      sizeof(lm_ivec2));
...
void lmSetHemisphereWeights(lm_context *ctx, lm_weight_func f, void *userdata) {
  // hemisphere weights texture. bakes in material dependent attenuation
  // behaviour.
  float *weights = (float *)LM_CALLOC(
      2 * 3 * ctx->hemisphere.size * ctx->hemisphere.size, sizeof(float));
  ...
  for (unsigned int i = 0;
       i < 2 * 3 * ctx->hemisphere.size * ctx->hemisphere.size; i++)
    weights[i] *= weightScale;

  // upload weight texture
  glBindTexture(GL_TEXTURE_2D, ctx->hemisphere.firstPass.weightsTexture);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RG32F, 3 * ctx->hemisphere.size,
               ctx->hemisphere.size, 0, GL_RG, GL_FLOAT, weights);
  LM_FREE(weights);
}

void lmSetTargetLightmap(lm_context *ctx, float *outLightmap, int w, int h,
                         int c) {
  ...
  if (ctx->hemisphere.storage.toLightmapLocation)
    LM_FREE(ctx->hemisphere.storage.toLightmapLocation);
  ctx->hemisphere.storage.toLightmapLocation =
      (lm_ivec2 *)LM_CALLOC(w * h, sizeof(lm_ivec2));
  ...
  if (ctx->lightmap.debug)
    LM_FREE(ctx->lightmap.debug);
  ctx->lightmap.debug =
      (unsigned char *)LM_CALLOC(ctx->lightmap.width * ctx->lightmap.height, 3);
#endif
}

void lmSetGeometry(lm_context *ctx, const float *transformationMatrix,
                   lm_type positionsType, const void *positionsXYZ,
                   int positionsStride, lm_type normalsType,
                   const void *normalsXYZ, int normalsStride,
                   lm_type lightmapCoordsType, const void *lightmapCoordsUV,
                   int lightmapCoordsStride, int count, lm_type indicesType,
                   const void *indices) {
  ...
  ctx->mesh.positionsStride =
      positionsStride == 0 ? sizeof(lm_vec3) : positionsStride;
  ctx->mesh.normals = (const unsigned char *)normalsXYZ;
  ctx->mesh.normalsType = normalsType;
  ctx->mesh.normalsStride =
      normalsStride == 0 ? sizeof(lm_vec3) : normalsStride;
  ctx->mesh.uvs = (const unsigned char *)lightmapCoordsUV;
  ctx->mesh.uvsType = lightmapCoordsType;
  ctx->mesh.uvsStride =
      lightmapCoordsStride == 0 ? sizeof(lm_vec2) : lightmapCoordsStride;
...
lm_bool lmBegin(lm_context *ctx, int *outViewport4, float *outView4x4,
                float *outProjection4x4) {
  assert(ctx->meshPosition.triangle.baseIndex < ctx->mesh.count);
  while (!lm_beginSampleHemisphere(
      ctx, outViewport4, outView4x4,
      outProjection4x4)) { // as long as there are no hemisphere sides to
                           // render...
    // try moving to the next rasterizer position
    if (lm_findNextConservativeTriangleRasterizerPosition(
            ctx)) { // if we successfully moved to the next sample position on
                    // the current triangle...
      ctx->meshPosition.hemisphere.side =
          0; // start sampling a hemisphere there
    } else { // if there are no valid sample positions on the current
             // triangle...
      if (ctx->meshPosition.triangle.baseIndex + 3 <
          ctx->mesh.count) { // ...and there are triangles left: move to the
                             // next triangle and continue sampling.
        lm_setMeshPosition(ctx, ctx->meshPosition.triangle.baseIndex + 3);
      } else { // ...and there are no triangles left: finish
        lm_integrateHemisphereBatch(ctx); // integrate and store last batch
        lm_writeResultsToLightmap(ctx); // read storage data from gpu memory and
                                        // write it to the lightmap

        if (++ctx->meshPosition.pass == ctx->meshPosition.passCount) {
          ctx->meshPosition.pass = 0;
          ctx->meshPosition.triangle.baseIndex =
              ctx->mesh.count; // set end condition (in case someone
                               // accidentally calls lmBegin again)

#ifdef LM_DEBUG_INTERPOLATION
          lmImageSaveTGAub("debug_interpolation.tga", ctx->lightmap.debug,
                           ctx->lightmap.width, ctx->lightmap.height, 3);
          ...
          for (int y = 0; y < ctx->lightmap.height; y++) {
            for (int x = 0; x < ctx->lightmap.width; x++) {
              if (ctx->lightmap.debug[(y * ctx->lightmap.width + x) * 3 + 0])
                rendered++;
              else if (ctx->lightmap
                           .debug[(y * ctx->lightmap.width + x) * 3 + 1])
                interpolated++;
              else
                wasted++;
            }
          ...
          printf("\n###########################################################"
                 "############\n");
          printf("%10d %6.2f%% rendered hemicubes integrated to lightmap "
                 "texels.\n",
                 rendered, 100.0f * (float)rendered / (float)total);
          printf("%10d %6.2f%% interpolated lightmap texels.\n", interpolated,
                 100.0f * (float)interpolated / (float)total);
          printf("%10d %6.2f%% wasted lightmap texels.\n", wasted,
                 100.0f * (float)wasted / (float)total);
...
float lmProgress(lm_context *ctx) {
  float passProgress =
      (float)ctx->meshPosition.triangle.baseIndex / (float)ctx->mesh.count;
  return ((float)ctx->meshPosition.pass + passProgress) /
         (float)ctx->meshPosition.passCount;
}

void lmEnd(lm_context *ctx) { lm_endSampleHemisphere(ctx); }

// these are not performance tuned since their impact on the whole lightmapping
// duration is insignificant
  ...
  for (int y = 0; y < h / 2; y++) {
    for (int x = 0; x < w / 2; x++) {
      ...
      for (int i = 0; i < c; i++) {
        ...
        sums[i] += image[p0 + i] + image[p0 + c + i] + image[p1 + i] +
                   image[p1 + c + i];
      }
...
void lmImageFtoUB(const float *image, unsigned char *outImage, int w, int h,
                  int c, float max) {
  assert(c > 0);
  float scale =
      255.0f /
      (max != 0.0f ? max : lmImageMax(image, w, h, c, LM_ALL_CHANNELS));
  for (int i = 0; i < w * h * c; i++)
    outImage[i] =
        (unsigned char)lm_minf(lm_maxf(image[i] * scale, 0.0f), 255.0f);
}
...
lm_bool lmImageSaveTGAf(const char *filename, const float *image, int w, int h,
                        int c, float max) {
  unsigned char *temp =
      (unsigned char *)LM_CALLOC(w * h * c, sizeof(unsigned char));
