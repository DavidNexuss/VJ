This is about compiling and linking applications that use GLFW. For information on how to write such applications, start with the \mbox{\hyperlink{quick_guide}{introductory tutorial}}. For information on how to compile the GLFW library itself, see \mbox{\hyperlink{compile_guide}{Compiling GLFW}}.

This is not a tutorial on compilation or linking. It assumes basic understanding of how to compile and link a C program as well as how to use the specific compiler of your chosen development environment. The compilation and linking process should be explained in your C programming material and in the documentation for your development environment.\hypertarget{build_guide_build_include}{}\doxysection{Including the GLFW header file}\label{build_guide_build_include}
...
This header defines all the constants and declares all the types and function prototypes of the GLFW API. By default, it also includes the Open\+GL header from your development environment. See \mbox{\hyperlink{build_guide_build_macros}{option macros}} below for how to select Open\+GL ES headers and more.
...
\end{DoxyCode}
\hypertarget{build_guide_build_macros}{}\doxysubsection{GLFW header option macros}\label{build_guide_build_macros}
...
A good general introduction to linking is \href{https://www.lurklurk.org/linkers/linkers.html}{\texttt{ Beginner\textquotesingle{}s Guide to Linkers}} by David Drysdale.\hypertarget{build_guide_build_link_win32}{}\doxysubsection{With Min\+GW or Visual C++ on Windows}\label{build_guide_build_link_win32}
...
The link library for the GLFW DLL is named {\ttfamily glfw3dll}. When compiling an application that uses the DLL version of GLFW, you need to define the \mbox{\hyperlink{build_guide_GLFW_DLL}{GLFW\+\_\+\+DLL}} macro {\itshape before} any inclusion of the GLFW header. This can be done either with a compiler switch or by defining it in your source code.\hypertarget{build_guide_build_link_cmake_source}{}\doxysubsection{With CMake and GLFW source}\label{build_guide_build_link_cmake_source}
This section is about using CMake to compile and link GLFW along with your application. If you want to use an installed binary instead, see \mbox{\hyperlink{build_guide_build_link_cmake_package}{With CMake and installed GLFW binaries}}.
...
Once GLFW has been added, link your application against the {\ttfamily glfw} target. This adds the GLFW library and its link-\/time dependencies as it is currently configured, the include directory for the GLFW header and, when applicable, the \mbox{\hyperlink{build_guide_GLFW_DLL}{GLFW\+\_\+\+DLL}} macro.
...
Note that the {\ttfamily glfw} target does not depend on Open\+GL, as GLFW loads any Open\+GL, Open\+GL ES or Vulkan libraries it needs at runtime. If your application calls Open\+GL directly, instead of using a modern \mbox{\hyperlink{context_guide_context_glext_auto}{extension loader library}}, use the Open\+GL CMake package.
...
For a minimal example of a program and GLFW sources built with CMake, see the \href{https://github.com/juliettef/GLFW-CMake-starter}{\texttt{ GLFW CMake Starter}} on Git\+Hub.\hypertarget{build_guide_build_link_cmake_package}{}\doxysubsection{With CMake and installed GLFW binaries}\label{build_guide_build_link_cmake_package}
This section is about using CMake to link GLFW after it has been built and installed. If you want to build it along with your application instead, see \mbox{\hyperlink{build_guide_build_link_cmake_source}{With CMake and GLFW source}}.
...
\begin{DoxyCode}{0}
\DoxyCodeLine{find\_package(glfw3 3.4 REQUIRED)}

\end{DoxyCode}


Once GLFW has been added to the project, link against it with the {\ttfamily glfw} target. This adds the GLFW library and its link-\/time dependencies, the include directory for the GLFW header and, when applicable, the \mbox{\hyperlink{build_guide_GLFW_DLL}{GLFW\+\_\+\+DLL}} macro.
...
The dependencies do not include Open\+GL, as GLFW loads any Open\+GL, Open\+GL ES or Vulkan libraries it needs at runtime. If your application calls Open\+GL directly, instead of using a modern \mbox{\hyperlink{context_guide_context_glext_auto}{extension loader library}}, you should add the {\ttfamily gl} pkg-\/config package.
...
\begin{DoxyNote}{Note}
Your machine may have {\ttfamily lib\+GL.$\ast$.dylib} style Open\+GL library, but that is for the X Window System and will not work with the mac\+OS native version of GLFW. 
\end{DoxyNote}
