...
Because GLFW 3 supports multiple windows, window handle parameters have been added to all window-\/related GLFW functions and callbacks. The handle of a newly created window is returned by glfw\+Create\+Window (formerly {\ttfamily glfw\+Open\+Window}). Window handles are pointers to the \href{https://en.wikipedia.org/wiki/Opaque_data_type}{\texttt{ opaque}} type GLFWwindow.

\begin{DoxyParagraph}{Old syntax}

\begin{DoxyCode}{0}
\DoxyCodeLine{glfwSetWindowTitle(\textcolor{stringliteral}{"{}New Window Title"{}});}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{glfwSetWindowTitle(window, \textcolor{stringliteral}{"{}New Window Title"{}});}

\end{DoxyCode}

\end{DoxyParagraph}
\hypertarget{moving_guide_moving_monitor}{}\doxysubsection{Explicit monitor selection}\label{moving_guide_moving_monitor}
GLFW 3 provides support for multiple monitors. To request a full screen mode window, instead of passing {\ttfamily GLFW\+\_\+\+FULLSCREEN} you specify which monitor you wish the window to use. The glfw\+Get\+Primary\+Monitor function returns the monitor that GLFW 2 would have selected, but there are many other \mbox{\hyperlink{monitor_guide}{monitor functions}}. Monitor handles are pointers to the \href{https://en.wikipedia.org/wiki/Opaque_data_type}{\texttt{ opaque}} type GLFWmonitor.
...
\begin{DoxyCode}{0}
\DoxyCodeLine{window = glfwCreateWindow(640, 480, \textcolor{stringliteral}{"{}My Window"{}}, glfwGetPrimaryMonitor(), NULL);}
...
GLFW 3 does not automatically poll for events in glfw\+Swap\+Buffers, meaning you need to call glfw\+Poll\+Events or glfw\+Wait\+Events yourself. Unlike buffer swap, which acts on a single window, the event processing functions act on all windows at once.
...
This means that you need to call glfw\+Make\+Context\+Current after creating a window before you can call any Open\+GL functions.\hypertarget{moving_guide_moving_hidpi}{}\doxysubsection{Separation of window and framebuffer sizes}\label{moving_guide_moving_hidpi}
Window positions and sizes now use screen coordinates, which may not be the same as pixels on machines with high-\/\+DPI monitors. This is important as Open\+GL uses pixels, not screen coordinates. For example, the rectangle specified with {\ttfamily gl\+Viewport} needs to use pixels. Therefore, framebuffer size functions have been added. You can retrieve the size of the framebuffer of a window with glfw\+Get\+Framebuffer\+Size function. A framebuffer size callback has also been added, which can be set with glfw\+Set\+Framebuffer\+Size\+Callback.

\begin{DoxyParagraph}{Old basic viewport setup}

\begin{DoxyCode}{0}
\DoxyCodeLine{glfwGetWindowSize(\&width, \&height);}
...
The {\ttfamily GLFW\+\_\+\+OPENED} window parameter has been removed. As long as the window has not been destroyed, whether through glfw\+Destroy\+Window or glfw\+Terminate, the window is \char`\"{}open\char`\"{}.
...
You can query the close flag at any time with glfw\+Window\+Should\+Close and set it at any time with glfw\+Set\+Window\+Should\+Close.
...
\end{DoxyParagraph}
The close callback no longer returns a value. Instead, it is called after the close flag has been set, so it can optionally override its value, before event processing completes. You may however not call glfw\+Destroy\+Window from the close callback (or any other window related callback).
...
The {\ttfamily glfw\+Open\+Window\+Hint} function has been renamed to glfw\+Window\+Hint.

Window hints are no longer reset to their default values on window creation, but instead retain their values until modified by glfw\+Window\+Hint or glfw\+Default\+Window\+Hints, or until the library is terminated and re-\/initialized.\hypertarget{moving_guide_moving_video_modes}{}\doxysubsection{Video mode enumeration}\label{moving_guide_moving_video_modes}
Video mode enumeration is now per-\/monitor. The glfw\+Get\+Video\+Modes function now returns all available modes for a specific monitor instead of requiring you to guess how large an array you need. The {\ttfamily glfw\+Get\+Desktop\+Mode} function, which had poorly defined behavior, has been replaced by glfw\+Get\+Video\+Mode, which returns the current mode of a monitor.\hypertarget{moving_guide_moving_char_up}{}\doxysubsection{Removal of character actions}\label{moving_guide_moving_char_up}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keywordtype}{void} character\_callback(GLFWwindow* window, \textcolor{keywordtype}{int} character);}

\end{DoxyCode}

\end{DoxyParagraph}
\hypertarget{moving_guide_moving_cursorpos}{}\doxysubsection{Cursor position changes}\label{moving_guide_moving_cursorpos}
The {\ttfamily glfw\+Get\+Mouse\+Pos} function has been renamed to glfw\+Get\+Cursor\+Pos, {\ttfamily glfw\+Set\+Mouse\+Pos} to glfw\+Set\+Cursor\+Pos and {\ttfamily glfw\+Set\+Mouse\+Pos\+Callback} to glfw\+Set\+Cursor\+Pos\+Callback.
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keywordtype}{void} scroll\_callback(GLFWwindow* window, \textcolor{keywordtype}{double} xoffset, \textcolor{keywordtype}{double} yoffset);}
...
The {\ttfamily GLFW\+\_\+\+KEY\+\_\+\+REPEAT} enable has been removed and key repeat is always enabled for both keys and characters. A new key action, {\ttfamily GLFW\+\_\+\+REPEAT}, has been added to allow the key callback to distinguish an initial key press from a repeat. Note that glfw\+Get\+Key still returns only {\ttfamily GLFW\+\_\+\+PRESS} or {\ttfamily GLFW\+\_\+\+RELEASE}.\hypertarget{moving_guide_moving_keys}{}\doxysubsection{Physical key input}\label{moving_guide_moving_keys}
...
The {\ttfamily glfw\+Get\+Joystick\+Pos} function has been renamed to glfw\+Get\+Joystick\+Axes.

The {\ttfamily glfw\+Get\+Joystick\+Param} function and the {\ttfamily GLFW\+\_\+\+PRESENT}, {\ttfamily GLFW\+\_\+\+AXES} and {\ttfamily GLFW\+\_\+\+BUTTONS} tokens have been replaced by the glfw\+Joystick\+Present function as well as axis and button counts returned by the glfw\+Get\+Joystick\+Axes and glfw\+Get\+Joystick\+Buttons functions.\hypertarget{moving_guide_moving_mbcs}{}\doxysubsection{Win32 MBCS support}\label{moving_guide_moving_mbcs}
...
GLFW 3 does not register glfw\+Terminate with {\ttfamily atexit} at initialization, because {\ttfamily exit} calls registered functions from the calling thread and while it is permitted to call {\ttfamily exit} from any thread, glfw\+Terminate must only be called from the main thread.

To release all resources allocated by GLFW, you should call glfw\+Terminate yourself, from the main thread, before the program terminates. Note that this destroys all windows not already destroyed with glfw\+Destroy\+Window, invalidating any window handles you may still have.\hypertarget{moving_guide_moving_glu}{}\doxysubsection{GLU header inclusion}\label{moving_guide_moving_glu}
...
\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ GLFW 2   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ GLFW 3   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Notes    }\\\cline{1-3}
\endhead
{\ttfamily glfw\+Open\+Window}   &glfw\+Create\+Window   &All channel bit depths are now hints    \\\cline{1-3}
{\ttfamily glfw\+Close\+Window}   &glfw\+Destroy\+Window   &\\\cline{1-3}
{\ttfamily glfw\+Open\+Window\+Hint}   &glfw\+Window\+Hint   &Now accepts all {\ttfamily GLFW\+\_\+$\ast$\+\_\+\+BITS} tokens    \\\cline{1-3}
{\ttfamily glfw\+Enable}   &glfw\+Set\+Input\+Mode   &\\\cline{1-3}
{\ttfamily glfw\+Disable}   &glfw\+Set\+Input\+Mode   &\\\cline{1-3}
{\ttfamily glfw\+Get\+Mouse\+Pos}   &glfw\+Get\+Cursor\+Pos   &\\\cline{1-3}
{\ttfamily glfw\+Set\+Mouse\+Pos}   &glfw\+Set\+Cursor\+Pos   &\\\cline{1-3}
{\ttfamily glfw\+Set\+Mouse\+Pos\+Callback}   &glfw\+Set\+Cursor\+Pos\+Callback   &\\\cline{1-3}
{\ttfamily glfw\+Set\+Mouse\+Wheel\+Callback}   &glfw\+Set\+Scroll\+Callback   &Accepts two-\/dimensional scroll offsets as doubles    \\\cline{1-3}
{\ttfamily glfw\+Get\+Joystick\+Pos}   &glfw\+Get\+Joystick\+Axes   &\\\cline{1-3}
{\ttfamily glfw\+Get\+Window\+Param}   &glfw\+Get\+Window\+Attrib   &\\\cline{1-3}
{\ttfamily glfw\+Get\+GLVersion}   &glfw\+Get\+Window\+Attrib   &Use {\ttfamily GLFW\+\_\+\+CONTEXT\+\_\+\+VERSION\+\_\+\+MAJOR}, {\ttfamily GLFW\+\_\+\+CONTEXT\+\_\+\+VERSION\+\_\+\+MINOR} and {\ttfamily GLFW\+\_\+\+CONTEXT\+\_\+\+REVISION}    \\\cline{1-3}
{\ttfamily glfw\+Get\+Desktop\+Mode}   &glfw\+Get\+Video\+Mode   &Returns the current mode of a monitor    \\\cline{1-3}
{\ttfamily glfw\+Get\+Joystick\+Param}   &glfw\+Joystick\+Present   &The axis and button counts are provided by glfw\+Get\+Joystick\+Axes and glfw\+Get\+Joystick\+Buttons   \\\cline{1-3}
\end{longtabu}
...
\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ GLFW 2   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ GLFW 3   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Notes    }\\\cline{1-3}
\endhead
{\ttfamily GLFWmousewheelfun}   &GLFWscrollfun   &\\\cline{1-3}
{\ttfamily GLFWmouseposfun}   &GLFWcursorposfun   &\\\cline{1-3}
\end{longtabu}
...
{\ttfamily GLFW\+\_\+\+MOUSE\+\_\+\+CURSOR}   &{\ttfamily GLFW\+\_\+\+CURSOR}   &Used with glfw\+Set\+Input\+Mode    \\\cline{1-3}
