...
CMake is a complex tool and this guide will only show a few of the possible ways to set up and compile GLFW. The CMake project has their own much more detailed \href{https://cmake.org/cmake/help/latest/guide/user-interaction/}{\texttt{ CMake user guide}} that includes everything in this guide not specific to GLFW. It may be a useful companion to this one.\hypertarget{compile_guide_compile_deps}{}\doxysubsection{Installing dependencies}\label{compile_guide_compile_deps}
The C/\+C++ development environments in Visual Studio, Xcode and Min\+GW come with all necessary dependencies for compiling GLFW, but on Unix-\/like systems like Linux and Free\+BSD you will need a few extra packages.\hypertarget{compile_guide_compile_deps_x11}{}\doxysubsubsection{Dependencies for X11}\label{compile_guide_compile_deps_x11}
...
On Debian and derivatives like Ubuntu and Linux Mint the {\ttfamily xorg-\/dev} meta-\/package pulls in the development packages for all of X11.
...
Once you have the required dependencies, move on to \mbox{\hyperlink{compile_guide_compile_generate}{Generating build files with CMake}}.\hypertarget{compile_guide_compile_deps_wayland}{}\doxysubsubsection{Dependencies for Wayland and X11}\label{compile_guide_compile_deps_wayland}
To compile GLFW for both Wayland and X11, you need to have the X11, Wayland and xkbcommon development packages installed. They are not needed to build or run programs that use GLFW. You will also need to set the \mbox{\hyperlink{compile_guide_GLFW_BUILD_WAYLAND}{GLFW\+\_\+\+BUILD\+\_\+\+WAYLAND}} CMake option in the next step when generating build files.

On Debian and derivatives like Ubuntu and Linux Mint you will need the {\ttfamily libwayland-\/dev}, {\ttfamily libxkbcommon-\/dev} and {\ttfamily wayland-\/protocols} packages and the {\ttfamily xorg-\/dev} meta-\/package. These will pull in all other dependencies.
...
On Free\+BSD you will need the {\ttfamily wayland}, {\ttfamily libxkbcommon} and {\ttfamily wayland-\/protocols} packages. The X11 headers are installed along the end-\/user X11 packages, so if you have an X server running you should have the headers as well. If not, install the {\ttfamily xorgproto} package.
...
Once you have the required dependencies, move on to \mbox{\hyperlink{compile_guide_compile_generate}{Generating build files with CMake}}.\hypertarget{compile_guide_compile_generate}{}\doxysubsection{Generating build files with CMake}\label{compile_guide_compile_generate}
...
A common pattern when building a single configuration is to have a build directory named {\ttfamily build} in the root of the source tree.\hypertarget{compile_guide_compile_generate_gui}{}\doxysubsubsection{Generating with the CMake GUI}\label{compile_guide_compile_generate_gui}
...
By default, GLFW will use X11 on Linux and other Unix-\/like systems other than mac\+OS. To include support for Wayland as well, set the \mbox{\hyperlink{compile_guide_GLFW_BUILD_WAYLAND}{GLFW\+\_\+\+BUILD\+\_\+\+WAYLAND}} option in the GLFW section of the variable list, then apply the new value as described above.
...
By default, GLFW will use X11 on Linux and other Unix-\/like systems other than mac\+OS. To also include support for Wayland, set the \mbox{\hyperlink{compile_guide_GLFW_BUILD_WAYLAND}{GLFW\+\_\+\+BUILD\+\_\+\+WAYLAND}} CMake option.
...
Once the GLFW library is compiled you are ready to build your application, linking it to the GLFW library. See \mbox{\hyperlink{build_guide}{Building applications}} for more information.\hypertarget{compile_guide_compile_options}{}\doxysection{CMake options}\label{compile_guide_compile_options}
...
\end{DoxyCode}
\hypertarget{compile_guide_compile_options_shared}{}\doxysubsection{Shared CMake options}\label{compile_guide_compile_options_shared}
\label{compile_guide_BUILD_SHARED_LIBS}%
\Hypertarget{compile_guide_BUILD_SHARED_LIBS}%
{\bfseries{BUILD\+\_\+\+SHARED\+\_\+\+LIBS}} determines whether GLFW is built as a static library or as a DLL / shared library / dynamic library. This is disabled by default, producing a static GLFW library. This variable has no {\ttfamily GLFW\+\_\+} prefix because it is defined by CMake. If you want to change the library only for GLFW when it is part of a larger project, see \mbox{\hyperlink{compile_guide_GLFW_LIBRARY_TYPE}{GLFW\+\_\+\+LIBRARY\+\_\+\+TYPE}}.

\label{compile_guide_GLFW_LIBRARY_TYPE}%
\Hypertarget{compile_guide_GLFW_LIBRARY_TYPE}%
{\bfseries{GLFW\+\_\+\+LIBRARY\+\_\+\+TYPE}} allows you to override \mbox{\hyperlink{compile_guide_BUILD_SHARED_LIBS}{BUILD\+\_\+\+SHARED\+\_\+\+LIBS}} only for GLFW, without affecting other libraries in a larger project. When set, the value of this option must be a valid CMake library type. Set it to {\ttfamily STATIC} to build GLFW as a static library, {\ttfamily SHARED} to build it as a shared library / dynamic library / DLL, or {\ttfamily OBJECT} to make GLFW a CMake object library.
...
{\bfseries{GLFW\+\_\+\+USE\+\_\+\+HYBRID\+\_\+\+HPG}} determines whether to export the {\ttfamily Nv\+Optimus\+Enablement} and {\ttfamily Amd\+Power\+Xpress\+Request\+High\+Performance} symbols, which force the use of the high-\/performance GPU on Nvidia Optimus and AMD Power\+Xpress systems. These symbols need to be exported by the EXE to be detected by the driver, so the override will not work if GLFW is built as a DLL. This is disabled by default, letting the operating system and driver decide.\hypertarget{compile_guide_compile_options_macos}{}\doxysubsection{mac\+OS specific CMake options}\label{compile_guide_compile_options_macos}
\label{compile_guide_GLFW_BUILD_COCOA}%
\Hypertarget{compile_guide_GLFW_BUILD_COCOA}%
{\bfseries{GLFW\+\_\+\+BUILD\+\_\+\+COCOA}} determines whether to include support for Cocoa when compiling the library. This option is only available when compiling for mac\+OS. This is enabled by default.\hypertarget{compile_guide_compile_options_unix}{}\doxysubsection{Unix-\/like system specific CMake options}\label{compile_guide_compile_options_unix}
\label{compile_guide_GLFW_BUILD_WAYLAND}%
\Hypertarget{compile_guide_GLFW_BUILD_WAYLAND}%
{\bfseries{GLFW\+\_\+\+BUILD\+\_\+\+WAYLAND}} determines whether to include support for Wayland when compiling the library. This option is only available when compiling for Linux and other Unix-\/like systems other than mac\+OS. This is disabled by default.

\label{compile_guide_GLFW_BUILD_X11}%
\Hypertarget{compile_guide_GLFW_BUILD_X11}%
{\bfseries{GLFW\+\_\+\+BUILD\+\_\+\+X11}} determines whether to include support for X11 when compiling the library. This option is only available when compiling for Linux and other Unix-\/like systems other than mac\+OS. This is enabled by default.\hypertarget{compile_guide_compile_mingw_cross}{}\doxysection{Cross-\/compilation with CMake and Min\+GW}\label{compile_guide_compile_mingw_cross}
...
For more details see the \href{https://cmake.org/cmake/help/latest/manual/cmake-toolchains.7.html}{\texttt{ CMake toolchain guide}}.\hypertarget{compile_guide_compile_manual}{}\doxysection{Compiling GLFW manually}\label{compile_guide_compile_manual}
...
\item {\bfseries{\+\_\+\+GLFW\+\_\+\+WAYLAND}} to use the Wayland API (incomplete)
\end{DoxyItemize}
